...
import {
  FaAndroid, FaPython, FaDatabase, FaFigma, FaCode, FaJava, FaBeer
} from "react-icons/fa";
...
const JobOpening = () => {
  ...
  const { name, email, resumeUrl, address, phoneNumber } = useSelector((state) => state.updateuser);
  ...
  useEffect(() => {
    ...
    axios.get(`/api/job_applied/candidate/${candidateId}`)
      .then((response) => {
  ...
  return (
    <>
      <UserDashboardHeader />
      <div className={`transition-all ${darkMode ? "bg-gray-900 text-white" : "bg-gray-100 text-black"}`}>
        <div className={`relative p-8 text-center rounded-lg ${darkMode ? "bg-gray-800" : "bg-blue-100"}`}>
        ...
        <div className="p-6 grid grid-cols-3 gap-4">
          {jobCategories.map((category) => (
            <button key={category.name} className={`flex flex-col items-center justify-center p-6 border rounded-lg shadow-md transition duration-300 cursor-pointer hover:bg-[#48596f] hover:text-white ${darkMode ? "bg-gray-700 text-white" : "bg-white"} ${filters.category === category.name ? "bg-blue-500" : ""}`}
        ...
        <div className="flex mt-3">
          <div className={`w-1/4 p-4 border-r ${darkMode ? "border-gray-600" : "border-gray-300"}`}>
            ...
            <button onClick={() => setFilters({ category: "", location: "" })} className="mt-4 px-4 py-2 bg-red-500 text-white rounded w-full cursor-pointer">
              Show All Jobs
            </button>
          </div>
          ...
          <div className="w-3/4 p-4 h-[40vh] overflow-y-scroll">
            {filteredJobs.length > 0 ? (
              filteredJobs.map((job) => (
                <div key={job._id} className={`p-4 border rounded-lg shadow-md mb-4 flex justify-between items-end ${darkMode ? "bg-gray-800" : "bg-white"}`}>
                  <div>
